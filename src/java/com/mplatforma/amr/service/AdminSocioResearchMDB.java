...
public class AdminSocioResearchMDB implements MessageListener {
    ...
    private static String INDEX_NAME = "databankalliance";
...
//    }
    ...
    private void init()
    {
        node = nodeBuilder().clusterName("elasticsearch_"+INDEX_NAME+"Prj_Cluster").client(false).node();
        try {
    ...
    private void perform_var_bulk_indexing()
    {
        try {
             if(vars_waiting_indexing!=null)
             {
                ...
                {
                    bulkRequest.add(client.prepareIndex(INDEX_NAME, "sociovar",String.valueOf(dto.getId()))
...
//            .actionGet();
            ...
            IndexResponse response = client.prepareIndex(INDEX_NAME, "sociovar",String.valueOf(dto.getId()))
            .setSource(generateVarJSONDesc(dto))
            .execute()
            .actionGet();
...
//            .actionGet();
            ...
            IndexResponse response = client.prepareIndex(INDEX_NAME, "law",String.valueOf(dto.getId()))
...
//                 .actionGet();
    ...
    private void perform_indexing_pub(PublicationDTO dto)
    {
        try {
            Client client = node.client();
            IndexResponse response = client.prepareIndex(INDEX_NAME, "publication",String.valueOf(dto.getId()))
            .setSource(dto.getJson_desctiptor())
            .execute()
            .actionGet();
            System.out.println(response.toString());
    ...
    private void perform_indexing_jury(ConsultationDTO dto)
    {
        try {
            Client client = node.client();
            IndexResponse response = client.prepareIndex(INDEX_NAME, "consultation",String.valueOf(dto.getId()))
            .setSource(dto.getJson_desctiptor())
            .execute()
            .actionGet();
            System.out.println(response.toString());
